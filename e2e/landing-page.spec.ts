...
test.describe('PRISMA Landing Page', () => {
  ...
  test('should display PRISMA landing page with correct branding', async ({ page }) => {
    await helpers.goto('/');
    ...
    await expect(page.getByRole('button', { name: /prueba gratuita/i })).toBeVisible();
  ...
  test('should handle geolocation-based language switching with flags', async ({ page }) => {
    await helpers.goto('/');
  ...
  test('should handle currency switching based on geolocation', async ({ page }) => {
    await helpers.goto('/');
    ...
    const currencyButton = page.locator('button').filter({ hasText: /\$.*(?:MXN|USD)/ }).first();
  ...
  test('should handle dark mode toggle', async ({ page }) => {
    ...
    const darkModeButton = page.locator('button').filter({ has: page.locator('svg') }).filter({ hasText: '' }).nth(2);
    ...
    await expect(page.locator('body')).toHaveClass(/dark:bg-gray-900|bg-gray-900/);
  ...
  test('should be responsive on mobile with PRISMA navigation', async ({ page }) => {
    ...
    await expect(page.locator('button').filter({ hasText: /Currency/ })).toBeVisible();
