...
export class TestHelpers {
  ...
  async signUp(email: string, password: string, fullName?: string) {
    ...
    await this.page.waitForURL(/\/dashboard/, { timeout: 10000 }).catch(() => {
      ...
      return this.page.waitForSelector('text=¡Cuenta Creada!', { timeout: 5000 })
    })
  }
  ...
  async signOut() {
    ...
    await this.page.locator('button').filter({ has: this.page.locator('svg') }).filter({ hasText: /User|Profile/ }).click()
  ...
  async createPortfolio(portfolioData: typeof testPortfolio) {
    ...
    await this.page.getByRole('button', { name: /Crear.*Portafolio|Create.*Portfolio/ }).click()
    
    // Fill portfolio form
    await this.page.getByPlaceholder(/Nombre.*portafolio|Portfolio.*name/i).fill(portfolioData.title)
    await this.page.getByPlaceholder(/Biografía|Bio/i).fill(portfolioData.bio)
    await this.page.getByPlaceholder(/Tagline|Lema/i).fill(portfolioData.tagline)
    
    // Save portfolio
    await this.page.getByRole('button', { name: /Guardar|Save/ }).click()
    
    // Wait for save confirmation or redirect
    await this.page.waitForSelector('text=guardado|saved|creado|created', { timeout: 5000 }).catch(() => {
  ...
  async publishPortfolio() {
    ...
    const confirmButton = this.page.getByRole('button', { name: /Confirmar|Confirm/ })
    if (await confirmButton.isVisible({ timeout: 2000 })) {
      await confirmButton.click()
    }
    
    // Wait for publish confirmation
    await this.page.waitForSelector('text=publicado|published|live', { timeout: 10000 })
  }
  ...
  async submitForm(submitButtonText = 'Enviar|Submit|Guardar|Save') {
    ...
    const submitButton = this.page.getByRole('button', { name: new RegExp(submitButtonText, 'i') })
  ...
  async waitForModal(modalIdentifier?: string) {
    ...
    throw new Error(`Modal not found${modalIdentifier ? ` for: ${modalIdentifier}` : ''}`)
  }
