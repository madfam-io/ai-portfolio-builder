...
export default function ExperimentDetailsPage(): React.ReactElement {
  ...
  const [experiment, setExperiment] = useState<LandingPageExperiment | null>(null);
  ...
  const [analyticsData, setAnalyticsData] = useState<ExperimentAnalyticsResponse | null>(null);
  ...
  const fetchExperimentData = async () => {
    try {
      ...
      const processedVariants = variantsData.map(variant => {
        ...
        const uniqueVisitors = new Set(analytics.map((a: any) => a.visitor_id || a.session_id)).size;
        const totalClicks = analytics.reduce((sum: number, a: any) => sum + (a.clicks?.length || 0), 0);
        const averageTimeOnPage = analytics.reduce((sum: number, a: any) => sum + (a.time_on_page || 0), 0) / (analytics.length || 1);
        const bounceRate = analytics.filter((a: any) => !a.clicks || a.clicks.length === 0).length / (analytics.length || 1) * 100;
        
        // Group conversions by day
        const conversionsByDay: Record<string, { conversions: number; visitors: number }> = {};
        ...
        return {
          ...
          analytics: {
            ...
            averageTimeOnPage,
            bounceRate,
            conversionsByDay: Object.entries(conversionsByDay).map(([date, data]) => ({
              date,
              ...data
  ...
  const calculateExperimentResults = (variants: DetailedVariant[]): any => {
    ...
    const variantResults: VariantResult[] = variants.map(variant => {
      ...
      const controlConversionRate = control.visitors > 0 ? (control.conversions / control.visitors) * 100 : 0;
      const uplift = control.id === variant.id ? 0 : ((conversionRate - controlConversionRate) / controlConversionRate) * 100;
      ...
      return {
        ...
        conversionRate,
        confidenceInterval: [Math.max(0, conversionRate - margin), conversionRate + margin] as [number, number],
        uplift,
    ...
    const significantResults = variantResults.filter(v => v.pValue && v.pValue < 0.05 && v.uplift && v.uplift > 0);
    const winner = significantResults.length > 0 
      ? significantResults.reduce((best, current) => current.uplift! > best.uplift! ? current : best)
      : null;
    ...
    return {
      ...
      duration: experiment ? Math.ceil((Date.now() - new Date(experiment.createdAt).getTime()) / (1000 * 60 * 60 * 24)) : 0
    };
  };

  // Calculate p-value (simplified - would use proper statistical library)
  const calculatePValue = (control: DetailedVariant, variant: DetailedVariant): number => {
    const p1 = control.conversions / control.visitors;
    const p2 = variant.conversions / variant.visitors;
    const p = (control.conversions + variant.conversions) / (control.visitors + variant.visitors);
  ...
  const generateTimeline = (variants: DetailedVariant[], range: string): any[] => {
    ...
    while (currentDate <= new Date()) {
      ...
      variants.forEach(variant => {
        const dayAnalytics = variant.analytics.conversionsByDay.find(d => d.date === dateStr);
        if (dayAnalytics) {
  ...
  const handleExportData = () => {
    ...
    const csvData = [
      ['Variant', 'Visitors', 'Conversions', 'Conversion Rate', 'Uplift', 'P-Value'],
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">
                  {analyticsData?.results.totalConversions.toLocaleString() || 0}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">
                  {analyticsData?.results.totalVisitors 
                    ? ((analyticsData.results.totalConversions / analyticsData.results.totalVisitors) * 100).toFixed(2)
                    : '0'
                  }%
                </p>
              </div>
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {analyticsData?.results.variantResults.map((result) => {
                  ...
                  return (
                    <tr key={result.variantId} className={result.variantId === analyticsData.results.winner ? 'bg-green-50 dark:bg-green-900/20' : ''}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {result.uplift !== undefined && result.uplift !== 0 ? (
                          <span className={`font-medium ${result.uplift > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {result.pValue && result.pValue < 0.05 ? (
                          <span className="inline-flex items-center gap-1 text-green-600 dark:text-green-400">
                            ...
                            <span className="text-sm">p={result.pValue.toFixed(3)}</span>
                          </span>
                        ) : (
                          <span className="text-sm text-gray-500 dark:text-gray-400">
                            {result.pValue ? `p=${result.pValue.toFixed(3)}` : '—'}
                          </span>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
