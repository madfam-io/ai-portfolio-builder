...
import { BioContext, ProjectEnhancement, TemplateRecommendation, UserProfile, EnhancedContent } from './types';
...
export class AIClient {
  ...
  async enhanceBio(bio: string, context: BioContext): Promise<EnhancedContent> {
    return this.makeRequest('/api/ai/enhance-bio', {
      method: 'POST',
      body: JSON.stringify({ bio, context }),
    });
  }
  ...
  async optimizeProject(
    ...
    return this.makeRequest('/api/ai/optimize-project', {
      method: 'POST',
      body: JSON.stringify({ title, description, technologies, context }),
    });
  }
   ...
   */
  async optimizeProjectsBatch(projects: Array<{
    title: string;
    description: string;
    technologies: string[];
  }>): Promise<{
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          throw new AIClientError('Authentication required', 'UNAUTHORIZED', false);
        }
        ...
        if (response.status === 503) {
          throw new AIClientError('Service temporarily unavailable', 'SERVICE_UNAVAILABLE', true);
        }
...
export const AIUtils = {
  ...
  extractUserProfile(portfolio: any): UserProfile {
    return {
      ...
      hasDesignWork: portfolio.projects?.some((p: any) => 
        p.technologies?.some((t: string) => 
          ['figma', 'sketch', 'photoshop', 'illustrator', 'design'].some(design => 
            t.toLowerCase().includes(design)
          )
        )
      ) || false,
      industry: portfolio.experience?.[0]?.company || undefined,
      experienceLevel: this.calculateExperienceLevel(portfolio.experience || []),
    };
  },
   ...
   */
  calculateExperienceLevel(experience: any[]): 'entry' | 'mid' | 'senior' | 'lead' {
  ...
  prepareBioContext(portfolio: any): BioContext {
    return {
      ...
      experience: portfolio.experience?.map((exp: any) => ({
        ...
        yearsExperience: this.calculateYearsExperience(exp.startDate, exp.endDate, exp.current),
      })) || [],
      industry: portfolio.experience?.[0]?.company || undefined,
   ...
   */
  calculateYearsExperience(startDate: string, endDate: string | null, current: boolean): number {
  ...
  needsEnhancement(content: string, type: 'bio' | 'project'): boolean {
    ...
    const hasQualityIndicators = qualityIndicators.some(pattern => pattern.test(content));
