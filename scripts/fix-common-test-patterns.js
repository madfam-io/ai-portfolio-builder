...
testFiles.forEach(file => {
  ...
  if (file.includes('/ai/') && content.includes('expect(data.models)')) {
    content = content.replace(/expect\(data\.models\)/g, 'expect(data.data.models)');
    fileFixed = true;
  ...
  if (content.match(/import.*act.*act.*act/)) {
    content = content.replace(/import\s*{([^}]*?)act\s*,\s*act\s*,\s*act([^}]*?)}/g, 'import {$1act$2}');
    fileFixed = true;
    console.log(`  ✓ Fixed duplicate act imports in ${file}`);
  }
  
  // Fix 3: Fix duplicate jest imports
  if (content.match(/import.*jest.*@jest\/globals.*\n.*import.*jest.*@jest\/globals/)) {
  ...
  if (content.includes('expect(body.error).toBe(') && content.includes('Too Many Requests')) {
