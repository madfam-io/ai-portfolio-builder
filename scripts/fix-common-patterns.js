...
middlewareFiles.forEach(file => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    content = content.replace(
      /await import\('next\/server'\)/g,
      "require('next/server')"
    );

    // Fix middleware function calls
    content = content.replace(
      /middleware\(request\)/g,
...
const onboardingTest = path.join(
  __dirname,
  '..',
...
if (fs.existsSync(signozTest)) {
  ...
  if (!content.includes('jest.mock')) {
    content =
      `jest.mock('@/lib/monitoring/signoz', () => ({
...
const templateConfigTest = path.join(
  __dirname,
  '..',
  '__tests__/lib/templates/templateConfig.test.ts'
);
