...
console.log(
  `Found ${testFiles.length} test files to check for class mocking issues...`
);
...
testFiles.forEach(file => {
  ...
  if (
    ...
    content.includes('HuggingFaceService.mockImplementation')
  ) {
  ...
  const servicePattern =
    /jest\.mock\('(@\/lib\/[^']+)'\);\s*.*?(\w+Service)\.mockImplementation/gs;
  if (servicePattern.test(content)) {
    content = content.replace(
      servicePattern,
      (match, importPath, serviceName) => {
