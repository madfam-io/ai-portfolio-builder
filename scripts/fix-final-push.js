...
middlewareTests.forEach(file => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (
      !content.includes('import { middleware }') &&
      !content.includes('const { middleware }')
    ) {
...
experimentFiles.forEach(file => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (
      ...
      content.includes('validateExperiment')
    ) {
...
`;
      ...
      if (
        !content.includes('const calculateResults') &&
        !content.includes('import { calculateResults')
      ) {
...
const demoPortfolioTest = path.join(
  __dirname,
  '..',
...
if (fs.existsSync(demoPortfolioTest)) {
  ...
  if (!content.includes('jest.mock')) {
    content =
      `jest.mock('@/lib/services/demo-portfolio-service', () => ({
...
function addConsoleMocks(content) {
  if (!content.includes('jest.spyOn(console')) {
    ...
    jest.spyOn(console, 'warn').mockImplementation(() => undefined);`;
      ...
      content =
        content.slice(0, insertPos) + consoleMocks + content.slice(insertPos);
    }
  }
  return content;
}
  ...
  files.forEach(file => {
    ...
    if (stat.isDirectory()) {
      ...
      if (newContent !== content) {
        ...
        console.log(
          `  ✅ Added console mocks to ${path.relative(path.join(__dirname, '..'), filePath)}`
        );
      }
    }
  });
