...
console.log(`Found ${testFiles.length} test files to check for timeout syntax...`);
...
testFiles.forEach((file) => {
  ...
  const timeoutPattern = /it\((['"`].*?['"`]),\s*async\s*\(,\s*(\d+)\)\s*=>\s*\{/g;
  ...
  const timeoutPattern2 = /it\((['"`].*?['"`]),\s*(\d+),\s*async\s*\(\)\s*=>\s*\{/g;
  ...
  // Pattern: it('description'(20000), async () => {
  const timeoutPattern3 = /it\((['"`].*?['"`])\s*\(\s*(\d+)\s*\),\s*async\s*\(\)\s*=>\s*\{/g;
