...
const fixes = [
  // Add jest imports
  {
    ...
    condition: (content) => !content.includes("from '@jest/globals'") && content.includes('@testing-library/react')
  },
  ...
  // Add missing act import
  {
    ...
    condition: (content) => content.includes('act(') && !content.includes('import.*act.*from')
  },
  // Fix useLanguage mock
  {
    pattern: /mockUseLanguage\.mockReturnValue/g,
    replacement: '(useLanguage as jest.MockedFunction<typeof useLanguage>).mockReturnValue',
  ...
  // Add jest environment pragma
  {
    ...
    condition: (content) => !content.includes('@jest-environment') && content.includes('.tsx"')
  },
  {
    pattern: /^/,
    replacement: `/**\n * @jest-environment node\n */\n\n`,
    condition: (content) => !content.includes('@jest-environment') && content.includes('route.test.ts')
  }
];
