...
try {
  ...
  const result = execSync(
    'npm test -- --passWithNoTests --verbose=false 2>&1 || true',
    {
      encoding: 'utf8',
  ...
  console.log(
    `   📊 Pass Rate: ${((suitesPassed / suitesTotal) * 100).toFixed(1)}%`
  );
  ...
  console.log(
    `   📊 Pass Rate: ${((testsPassed / testsTotal) * 100).toFixed(1)}%`
  );
  ...
  console.log(
    `   Current State: ${((suitesPassed / suitesTotal) * 100).toFixed(1)}% pass rate (${suitesPassed}/${suitesTotal} suites)`
  );
  console.log(
    `   📈 Improvement: +${((suitesPassed / suitesTotal) * 100).toFixed(1)}%`
  );
