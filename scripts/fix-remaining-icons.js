...
function processFile(filePath) {
  ...
  try {
    ...
    if (content.includes(`from 'react-icons/`)) {
      ...
      content = content.replace(/import\s*{\s*[^}]+\s*}\s*from\s*['"]react-icons\/\w+['"]\s*;?\s*\n?/g, '');
      ...
      if (lucideImports.size > 0) {
        ...
        if (existingLucideImport) {
          ...
          const existingIcons = existingLucideImport[1].split(',').map(i => i.trim());
          const allIcons = [...new Set([...existingIcons, ...lucideImports])].sort();
          ...
          if (firstImportMatch) {
            ...
            content = content.slice(0, insertPosition) + `\nimport { ${lucideIconsList} } from 'lucide-react';` + content.slice(insertPosition);
          }
        }
        modified = true;
      }
