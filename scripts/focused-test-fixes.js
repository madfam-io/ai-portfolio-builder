...
testFiles.forEach(file => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (
      ...
      content.match(/mockUseLanguage.*mockUseLanguage/)
    ) {
      const lines = content.split('\n');
      const cleanLines = lines.filter((line, index) => {
        if (line.includes('mockUseLanguage') && line.includes('import')) {
          ...
          const prevMockImports = lines
            .slice(0, index)
            .filter(l => l.includes('mockUseLanguage') && l.includes('import'));
...
commonProblems.forEach(file => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (
      content.includes('useLanguage: () => mockUseLanguage()') &&
      content.includes('useLanguage: jest.fn()')
    ) {
