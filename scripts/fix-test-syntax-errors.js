...
testFiles.forEach(filePath => {
  ...
  const useLanguageMockPattern = /jest\.mock\('@\/lib\/i18n\/refactored-context'[^}]+}\);?\s*\n?/gs;
  ...
  content = content.replace(/describe\([^{]+{\s*,\s*$/gm, match => match.replace(',', ''));
  ...
  for (let i = 1; i < mockBlocks.length; i++) {
    ...
    if (closeIndex === -1) {
      ...
      if (nextMockIndex > -1) insertIndex = Math.min(insertIndex, nextMockIndex);
      if (describeIndex > -1) insertIndex = Math.min(insertIndex, describeIndex);
      
      // Find the last }) before the insertIndex
      const lastBraceIndex = block.lastIndexOf('})', insertIndex);
      if (lastBraceIndex > -1) {
        block = block.slice(0, lastBraceIndex + 2) + ');' + block.slice(lastBraceIndex + 2);
        modified = true;
