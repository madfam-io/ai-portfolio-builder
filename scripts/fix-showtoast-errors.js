...
testFiles.forEach(file => {
  ...
  if (content.includes('showToast') && !content.includes("jest.mock('@/lib/store/ui-store'")) {
    // Add ui-store mock before the first describe or test
    const insertPosition = content.indexOf('describe(') || content.indexOf('test(') || content.indexOf('it(');
    if (insertPosition > 0) {
...
`;
  ...
  if (file.includes('store') && content.includes('Assignment to constant variable')) {
