...
testFiles.forEach(file => {
  ...
  if (content.includes('ULTIMATE TEST SETUP')) {
    const setupStart = content.indexOf(
      '// ==================== ULTIMATE TEST SETUP ===================='
    );
    ...
    if (setupStart !== -1 && setupEnd !== -1) {
      ...
      const afterSetup = content.substring(
        setupEnd +
          '// ==================== END ULTIMATE SETUP ===================='
            .length
      );
...
try {
  const testResult = execSync(
    'pnpm test --passWithNoTests 2>&1 | grep -E "(Tests:|Test Suites:|ERROR)" | head -5',
    {
      cwd: path.join(__dirname, '..'),
      encoding: 'utf8',
      timeout: 30000,
  ...
  console.log(
    ...
    testResult || 'No immediate errors detected'
  );
} catch (error) {
