...
`;
  ...
  fixed = fixed.replace(
    /async\s+function\s+([A-Z]+)\s*\(/g,
    'export async function $1('
  );

  // Fix incomplete statements
  fixed = fixed.replace(
    /export\s+async\s+function\s+[A-Z]+\s*\([^)]*\)\s*:\s*Promise<[^>]*>\s*{\s*$/gm,
    match => match + '\n  return new Response();\n}'
  );
    ...
    for (const filePath of files) {
      ...
      try {
        ...
        if (modified) {
          ...
          console.log(
            `✅ Fixed boolean expressions in: ${path.relative(projectRoot, filePath)}`
          );
        }
      } catch (error) {
        console.error(`❌ Error fixing ${filePath}:`, error.message);
