...
const specificFixes = {
  '__tests__/components/dashboard/ai-credit-packs.test.tsx': content => {
    ...
    content = content.replace(
      /import React from 'react';\s*import React from 'react';/g,
  ...
  '__tests__/integration/template-system.test.tsx': content => {
    ...
    // Fix the broken render line
    content = content.replace(
      /^\s*<TemplateRenderer[^>]+\/>/m,
      '      render(<TemplateRenderer portfolio={portfolioWithTemplate} />);'
    );
    return content;
  },
  ...
  '__tests__/components/editor/EditorContent.test.tsx': content => {
    ...
    content = content.replace(
      /import { mockUseLanguage } from '@\/__tests__\/utils\/mock-i18n'; '@\/__tests__\/utils\/mock-i18n';/g,
    ...
    content = content.replace(
      /\n\s*expect\(enhanceButton\)\.toHaveFocus\(\);[\s\S]*?\}\);[\s\S]*?\}\);/m,
      match => {
        return `
  ...
  '__tests__/lib/services/portfolio/portfolio-service.test.ts': content => {
    ...
    content = content.replace(
      /^d<PortfolioRepository>;/m,
      '    const mockRepository = {} as jest.Mocked<PortfolioRepository>;'
    );
    // Remove the orphaned closing braces
    content = content.replace(
      /^\s*\)\);\s*}\);\s*}\);[\s\S]*?describe\('getUserPortfolios'/m,
      "  describe('getUserPortfolios'"
    );
    // Fix missing closing parentheses
    content = content.replace(
      /expect\(mockRepository\.update\)\.toHaveBeenCalledWith\(\s*'portfolio-123',\s*updateDto\s*\n/g,
      "expect(mockRepository.update).toHaveBeenCalledWith(\n        'portfolio-123',\n        updateDto\n      );"
    );
    // Fix other missing parentheses
    content = content.replace(
      /expect\(service\.deletePortfolio\('portfolio-123'\)\)\.rejects\.toThrow\(\s*'Cannot delete published portfolio'\s*\n/g,
      "expect(service.deletePortfolio('portfolio-123')).rejects.toThrow(\n        'Cannot delete published portfolio'\n      );"
    );
    content = content.replace(
      /mockRepository\.findById\.mockRejectedValue\(\s*new Error\('Database error'\)\s*\n/g,
      "mockRepository.findById.mockRejectedValue(\n        new Error('Database error')\n      );"
    );
    content = content.replace(
      /await expect\(service\.getUserPortfolios\('user-123'\)\)\.rejects\.toThrow\(\s*'Database error'\s*\n/g,
  ...
  '__tests__/components/billing/upgrade-modal.test.tsx': content => {
    ...
    content = content.replace(
      /^\s*expect\(mockCreateCheckoutSession\)\.toHaveBeenCalledWith\({[\s\S]*?}\);[\s\S]*?}\);[\s\S]*?}\);/m,
      match => {
