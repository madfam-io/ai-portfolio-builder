...
testFiles.forEach(filePath => {
  ...
  if (!content.includes('@jest/globals')) {
    ...
    if (jestImports.length > 0) {
      ...
      const jsdomMatch = content.match(
  ...
  if (
    ...
    if (!content.includes('api-route-test-helpers')) {
      const importLine =
        "import { setupCommonMocks, createMockRequest } from '@/tests/utils/api-route-test-helpers';\n";
    ...
    content = content.replace(
      /jest\.mock\('@\/lib\/supabase\/server'[^}]+}\);?\s*\n?/gs,
      ''
    );
    ...
    if (!content.includes('setupCommonMocks()')) {
      ...
      if (describeMatch) {
        const insertPos =
          content.indexOf(describeMatch[0]) + describeMatch[0].length;
        content =
          content.slice(0, insertPos) +
          '  setupCommonMocks();\n\n' +
          content.slice(insertPos);
  ...
  if (
    ...
    const rtlImportMatch = content.match(
  ...
  if (
    ...
    if (
      (content.includes('useLanguage') ||
        ...
        content.includes('EditorSidebar') ||
...
`;
    ...
    if (
      content.includes('useApp') &&
      !content.includes("jest.mock('@/hooks/useApp')")
    ) {
      const mockCode = `
// Mock useApp hook
...
`;
      ...
      const languageMockMatch = content.match(
      ...
      if (languageMockMatch) {
        insertPos =
          content.indexOf(languageMockMatch[0]) + languageMockMatch[0].length;
      } else {
        const lastImportMatch = content.match(/^import.*from.*;$/gm);
        if (lastImportMatch) {
          const lastImport = lastImportMatch[lastImportMatch.length - 1];
      ...
      if (insertPos) {
        content =
          content.slice(0, insertPos) + mockCode + content.slice(insertPos);
        modified = true;
