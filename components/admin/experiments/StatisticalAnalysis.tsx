...
export default function StatisticalAnalysis({ 
  ...
  const getSampleSizeRecommendation = (): {
    ...
    if (currentSampleSize >= minSampleSize * 2) {
      ...
      return {
        status: 'warning',
        message: 'Sample size is adequate but more data would increase confidence'
      };
    } else {
      return {
        status: 'insufficient',
  ...
  const winningVariant = results.variantResults.find(v => v.variantId === results.winner);
          ...
          <div className="flex items-start gap-3 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
            ...
            <div>
              ...
              <p className="text-sm text-green-700 dark:text-green-300 mt-1">
                {winningVariant?.variantName} shows a {results.improvementPercentage?.toFixed(1)}% improvement
                over the control with {results.confidence?.toFixed(1)}% confidence.
              </p>
            </div>
          </div>
        ) : (
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-blue-700 dark:text-blue-300">
              {hasWinner ? (
                <>
                  <li>• Consider implementing {winningVariant?.variantName} as the new default</li>
                  <li>• Document learnings and apply insights to future experiments</li>
                  <li>• Monitor performance post-implementation to ensure sustained improvement</li>
                </>
              ) : (
                <>
                  <li>• Continue running the experiment for at least {Math.ceil(14 - results.duration)} more days</li>
                  <li>• Ensure equal traffic distribution between variants</li>
                  <li>• Consider testing more dramatic changes if no difference emerges</li>
                </>
              )}
            </ul>
          </div>
