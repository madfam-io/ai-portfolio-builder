...
import { Briefcase, Users, DollarSign, Mic, UserCheck, Globe } from 'lucide-react';
...
const audienceTypes: Array<{ value: AudienceType; label: string; icon: any; description: string }> = [
  {
    value: 'recruiter',
    label: 'Recruiter',
    icon: Briefcase,
...
export function CreateVariantDialog({
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: t.variantCreatedDesc || 'Your new portfolio variant has been created',
      });
  ...
  const generateVariantName = (audienceType: AudienceType) => {
    const typeLabel = audienceTypes.find(t => t.value === audienceType)?.label || 'General';
    ...
    <Dialog open={open} onOpenChange={(open) => {
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === 'type' 
              ? t.selectAudienceType || 'Select the type of audience this variant is for'
              : t.provideVariantDetails || 'Provide details about your target audience'}
          </DialogDescription>
        </DialogHeader>
        ...
        {step === 'type' ? (
          <div className="space-y-4">
            ...
            >
              {audienceTypes.map(({ value, label, icon: Icon, description }) => (
                <label
                  key={value}
                  htmlFor={value}
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder={t.variantNamePlaceholder || 'e.g., Tech Recruiter Version'}
              />
            </div>

            <div>
              <Label htmlFor="audienceName">{t.audienceName || 'Audience Name'} *</Label>
              <Input
                id="audienceName"
                value={formData.audienceName}
                onChange={(e) => setFormData({ ...formData, audienceName: e.target.value })}
                placeholder={t.audienceNamePlaceholder || 'e.g., FAANG Recruiters'}
              />
            </div>

            <div>
              <Label htmlFor="description">{t.audienceDescription || 'Audience Description'}</Label>
              <Textarea
                id="description"
                value={formData.description}
                onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                placeholder={t.audienceDescPlaceholder || 'Describe your target audience...'}
                rows={3}
              />
            </div>
            ...
            <div>
              <Label htmlFor="keyPriorities">{t.keyPriorities || 'Key Priorities'}</Label>
              <Input
                id="keyPriorities"
                value={formData.keyPriorities}
                onChange={(e) => setFormData({ ...formData, keyPriorities: e.target.value })}
                placeholder={t.keyPrioritiesPlaceholder || 'e.g., leadership, scalability, innovation'}
              />
          ...
          {step === 'type' ? (
            ...
            <>
              ...
              <Button onClick={handleCreate} disabled={isCreating}>
                {isCreating ? t.creating || 'Creating...' : t.createVariant || 'Create Variant'}
              </Button>
            </>
          )}
        </DialogFooter>
