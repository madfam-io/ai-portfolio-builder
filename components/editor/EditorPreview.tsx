            ...
            <div className="p-8">
              ...
              <div className="text-center mb-12">
                <h1 className="text-4xl font-bold mb-4">
                  {portfolio.data?.name || portfolio.name || t.yourName || 'Your Name'}
                </h1>
                <p className="text-xl text-muted-foreground mb-2">
                  {portfolio.data?.title || portfolio.data?.headline || t.yourTitle || 'Your Professional Title'}
                </p>
                <p className="text-lg text-muted-foreground">
                  {portfolio.data?.tagline || t.yourTagline || 'Your tagline goes here'}
                </p>
              </div>
              ...
              {portfolio.data?.about && (
                <div className="mb-12">
                  <h2 className="text-2xl font-semibold mb-4">{t.about || 'About'}</h2>
              ...
              {portfolio.data?.projects && portfolio.data.projects.length > 0 && (
                <div className="mb-12">
                  <h2 className="text-2xl font-semibold mb-4">{t.projects || 'Projects'}</h2>
                  <div className="grid gap-6">
                    {portfolio.data?.projects.map((project: any, index: number) => (
                      <div key={project.id || index} className="p-6 border rounded-lg">
                        <h3 className="text-lg font-semibold mb-2">{project.title}</h3>
                        <p className="text-muted-foreground">{project.description}</p>
                        {project.technologies && (
                          <div className="flex gap-2 mt-4">
                            {project.technologies.map((tech: string, i: number) => (
                              <span key={i} className="px-2 py-1 bg-primary/10 text-sm rounded">
                                {tech}
                              </span>
                            ))}
                          </div>
