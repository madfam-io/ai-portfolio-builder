 ...
 */
export const SafeIconWrapper: React.FC<SafeIconWrapperProps> = ({ iconType, className }) => {
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'data-icon-state') {
    ...
    const selector = iconType === 'theme' ? '[data-dark-mode-icon]' : '[data-mobile-menu-icon]';
    ...
    if (element) {
      ...
      observer.observe(element, { attributes: true, attributeFilter: ['data-icon-state'] });
    }
  ...
  if (iconType === 'theme') {
    return iconState === 'sun' ? <SunIcon className={className} /> : <MoonIcon className={className} />;
  } else {
    return iconState === 'close' ? <CloseIcon className={className} /> : <MenuIcon className={className} />;
  }
};
 ...
 */
export const MobileMenuIcon: React.FC<{ className?: string }> = ({ className }) => (
