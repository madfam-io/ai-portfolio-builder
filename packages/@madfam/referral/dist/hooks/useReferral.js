...
const logger = new logger$1.Logger("useReferral");
function useReferral(config) {
  ...
  const shareToSocial = react.useCallback(
  ...
  const selectCampaign = react.useCallback((campaign) => {
    setState((prev) => ({ ...prev, currentCampaign: campaign }));
  }, []);
  const refreshCampaigns = react.useCallback(async () => {
    await fetchCampaigns();
  }, [fetchCampaigns]);
  const refreshReferrals = react.useCallback(async () => {
