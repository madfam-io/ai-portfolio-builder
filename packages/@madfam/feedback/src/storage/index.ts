...
import type { StorageAdapter, StorageConfig, StorageError as StorageErrorType } from '../core/types';
...
export function createStorageAdapter(config?: StorageConfig): StorageAdapter {
  ...
  switch (storageType) {
    ...
    case 'supabase':
      if (!config?.connectionString && (!config?.options?.supabaseUrl || !config?.options?.supabaseKey)) {
