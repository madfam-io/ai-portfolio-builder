...
describe('GatewayFeeCalculator', () => {
  ...
  describe('calculateFees', () => {
    ...
    it('should calculate Stripe fees correctly', () => {
      ...
      expect(fees.processingFee.amount).toBe(3.20);
    });
    ...
    it('should calculate LemonSqueezy fees correctly', () => {
      ...
      expect(fees.fixedFee.amount).toBe(0.50);
      expect(fees.totalFee.amount).toBe(5.50);
    });
    ...
    it('should add international card fees', () => {
      ...
      expect(fees.totalFee.amount).toBe(4.20); // 2.9% + 0.30 + 1%
    });

    it('should calculate installment fees', () => {
      const fees = calculator.calculateFees('mercadopago', testAmount, false, 6);
    ...
    it('should convert currency for fixed fees', () => {
      ...
      expect(fees.fixedFee.amount).toBeGreaterThan(0.30); // More than USD value
    });
  });

  describe('getEffectiveRate', () => {
    it('should calculate effective rate including fixed fees', () => {
      const smallAmount: Money = { amount: 10, currency: 'USD', display: '$10' };
      const largeAmount: Money = { amount: 1000, currency: 'USD', display: '$1000' };
    ...
    it('should include international fees in effective rate', () => {
      ...
      const internationalRate = calculator.getEffectiveRate('stripe', amount, true);
  ...
  describe('compareGateways', () => {
    it('should compare multiple gateways and sort by cost', () => {
      const amount: Money = { amount: 100, currency: 'USD', display: '$100' };
      const gateways: Gateway[] = ['stripe', 'mercadopago', 'lemonsqueezy', 'paypal'];
  ...
  describe('installment rates', () => {
    it('should apply correct MercadoPago installment rates', () => {
      const amount: Money = { amount: 1000, currency: 'BRL', display: 'R$1000' };
      ...
      rates.forEach(({ months, expectedRate }) => {
        const fees = calculator.calculateFees('mercadopago', amount, false, months);
        expect(fees.installmentFee?.amount).toBeCloseTo(amount.amount * expectedRate);
      });
    });
