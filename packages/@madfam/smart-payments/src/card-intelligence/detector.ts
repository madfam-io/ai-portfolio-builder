...
const LOCAL_PAYMENT_METHODS: Record<string, LocalPaymentMethod[]> = {
  MX: [
    { id: 'oxxo', name: 'OXXO', type: 'cash', countries: ['MX'], gateway: 'mercadopago', processingTime: '1-2 days' },
    { id: 'spei', name: 'SPEI', type: 'bank_transfer', countries: ['MX'], gateway: 'mercadopago', processingTime: 'Instant' },
  ],
  BR: [
    { id: 'boleto', name: 'Boleto Bancário', type: 'cash', countries: ['BR'], gateway: 'mercadopago', processingTime: '1-3 days' },
    { id: 'pix', name: 'PIX', type: 'bank_transfer', countries: ['BR'], gateway: 'mercadopago', processingTime: 'Instant' },
  ],
  IN: [
    { id: 'upi', name: 'UPI', type: 'wallet', countries: ['IN'], gateway: 'razorpay', processingTime: 'Instant' },
    { id: 'netbanking', name: 'Net Banking', type: 'bank_transfer', countries: ['IN'], gateway: 'razorpay', processingTime: 'Instant' },
  ],
  AR: [
    { id: 'rapipago', name: 'Rapipago', type: 'cash', countries: ['AR'], gateway: 'mercadopago', processingTime: '1-2 days' },
    { id: 'pagofacil', name: 'Pago Fácil', type: 'cash', countries: ['AR'], gateway: 'mercadopago', processingTime: '1-2 days' },
  ],
};
...
export class CardDetector {
  ...
  async suggestOptimalGateway(cardInfo: CardInfo): Promise<Gateway> {
    ...
    const countryPriority = priorityMap[issuerCountry] || ['stripe', 'lemonsqueezy'];
  ...
  private async enhanceCardInfo(
    ...
    const supportedGateways = this.getSupportedGateways(issuerCountry, binInfo.brand || 'unknown');
  ...
  private getSupportedGateways(country: string, brand: CardBrand): Gateway[] {
    ...
    for (const [gateway, countries] of Object.entries(GATEWAY_COUNTRY_SUPPORT)) {
      if (countries.includes('*') || countries.includes(country)) {
        // Additional brand-specific checks
        if (gateway === 'paypal' && brand === 'amex' && !['US', 'CA'].includes(country)) {
