...
export class PriceManipulationDetector {
  ...
  detectManipulation(
    ...
    const confidence = this.calculateConfidence(evidence.riskScore, evidence.factors.length);
  ...
  private checkCardCountryMismatch(
    ...
    if (pricingDiscount > cardDiscount + 0.5) {
      factors.push('Extreme discount difference between card and pricing country');
      score += 30;
    }
  ...
  private checkSuspiciousBehavior(
    ...
    if (customer.country && geoContext.ipCountry && 
  ...
  private checkLocationSpoofing(
    ...
    // Timezone mismatch
    const expectedTimezone = this.getExpectedTimezone(geoContext.ipCountry || 'US');
    ...
    // Language mismatch
    const expectedLanguage = this.getExpectedLanguage(geoContext.ipCountry || 'US');
    if (geoContext.language !== expectedLanguage && 
        !this.isCommonSecondLanguage(geoContext.language, geoContext.ipCountry || 'US')) {
  ...
  private isCommonTravelPair(country1: string, country2: string): boolean {
    const pairs = [
      ['US', 'CA'], ['US', 'MX'], ['GB', 'FR'], ['GB', 'ES'],
      ['DE', 'FR'], ['DE', 'NL'], ['AU', 'NZ'], ['SG', 'MY'],
      ['AR', 'BR'], ['AR', 'CL'], ['IN', 'AE'], ['IN', 'SG'],
    ];
