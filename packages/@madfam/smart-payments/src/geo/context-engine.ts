...
export class GeographicalContextEngine {
  ...
  constructor(private config: ContextEngineConfig = {}) {
    ...
    this.suspiciousCountries = new Set(config.suspiciousCountries || [
      'NG', // Nigeria
      'GH', // Ghana
      'PK', // Pakistan
      'BD', // Bangladesh
  ...
  async buildContext(ipAddress: string, headers?: Record<string, string>): Promise<GeographicalContext> {
    ...
    try {
      ...
      throw new GeoLookupError(
        'Failed to build geographical context',
  ...
  assessRisk(
    ...
    const expectedTimezone = this.getExpectedTimezone(context.ipCountry || 'US');
    if (context.timezone && !this.isTimezoneMatch(context.timezone, expectedTimezone)) {
      factors.push({
   ...
   */
  async lookup(ipAddress: string, headers?: Record<string, string>): Promise<GeoLookupResponse> {
    try {
  ...
  private fallbackIPLookup(ipAddress: string): any {
    ...
    if (first >= 1 && first <= 50) return { countryCode: 'US', timezone: 'America/New_York' };
    if (first >= 51 && first <= 100) return { countryCode: 'EU', timezone: 'Europe/London' };
    if (first >= 101 && first <= 150) return { countryCode: 'AS', timezone: 'Asia/Tokyo' };
    if (first >= 151 && first <= 200) return { countryCode: 'BR', timezone: 'America/Sao_Paulo' };
  ...
  private calculateRiskScore(context: GeographicalContext, vpnCheck: VPNCheckResult | null): number {
    let score = 0;
  ...
  private generateRecommendation(risk: RiskAssessment['risk'], factors: RiskFactor[]): string {
    if (risk === 'critical') {
      return 'Block transaction and flag for manual review';
    }
    
    if (risk === 'high') {
      if (factors.some(f => f.type === 'vpn_detected' && f.severity === 'high')) {
