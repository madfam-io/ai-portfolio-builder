...
export class DynamicPricingEngine {
  ...
  async calculatePrice(context: PricingContext): Promise<PriceCalculation> {
    const { basePrice, cardCountry, ipCountry, vpnDetected, customer } =
      context;

    // Check for price manipulation
    let manipulationDetected = false;
    if (
      this.config.enableManipulationDetection !== false &&
      ...
      manipulationDetected =
        manipulationCheck.detected && manipulationCheck.confidence > 0.6;
    }
    ...
    // Calculate savings
    const savingsAmount =
      adjustment.type === 'discount' ? adjustment.amount : undefined;
    ...
    return {
      ...
      cardCountryPrice: cardCountry
        ? await this.getCountryPrice(basePrice, cardCountry)
        : undefined,
      reasoning,
  ...
  private getEligibleDiscounts(
    ...
    return this.discounts.filter(discount => {
      ...
      if (
        discount.eligibility.countries &&
  ...
  private calculateAdjustment(
    ...
    let adjustmentPercentage = Math.max(
      pppAdjustment,
      bestDiscount.amount / 100
    );
    ...
    if (adjustmentPercentage === 0) {
      return {
        ...
        percentage: 0,
        amount: basePrice,
        reason: 'No adjustment applicable',
      };
    }
  ...
  private generateReasoning(
    ...
    if (manipulationDetected) {
      ...
      if (adjustment && adjustment.type === 'discount') {
        factors.push(
          `${(adjustment.percentage * 100).toFixed(0)}% regional discount applied`
        );
      }
