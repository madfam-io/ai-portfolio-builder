...
import { DEFAULT_GATEWAY_CONFIGS, GatewayFeeCalculator } from './gateway-config';
...
export class IntelligentRouter {
  ...
  async route(context: PaymentContext): Promise<PaymentOptions> {
    ...
    // Score each gateway
    const scoredGateways = await this.scoreGateways(
      availableGateways,
      context
    );
    ...
    return {
      ...
      pricingContext: context.pricingContext || {
        ...
        basePrice: amount,
        reasoning: { applied: 'base_price', explanation: 'Base pricing applied', factors: [] },
  ...
  private async getAvailableGateways(
    ...
    if (cardInfo) {
      gateways = gateways.filter(g => 
        cardInfo.supportedGateways.includes(g)
      );
    }
    ...
    if (geoContext?.ipCountry) {
      gateways = gateways.filter(g => {
        ...
        return config.supportedCountries.includes('*') || 
    ...
    if (this.config.blockHighRiskGateways) {
      gateways = gateways.filter(g => 
        !this.config.blockHighRiskGateways!.includes(g)
      );
    }
  ...
  private async buildGatewayOptions(
    ...
    return Promise.all(scoredGateways.map(async ({ gateway, factors }) => {
      const config = DEFAULT_GATEWAY_CONFIGS[gateway];
      ...
      const estimatedTotal: Money = {
        ...
        currency: amount.currency,
        display: this.formatMoney(amount.amount + fees.totalFee.amount, amount.currency),
      };
      ...
      const installmentOptions = this.isInstallmentEligible(context) && 
                                config.features.supportsInstallments
        ? this.getInstallmentOptions(gateway, amount)
        : undefined;
      
      // Get trust signals
      const trustSignals = this.getTrustSignals(gateway, geoContext?.ipCountry);
  ...
  private getInstallmentRate(gateway: Gateway, months: number): number {
    const rates: Record<Gateway, Record<number, number>> = {
      mercadopago: { 3: 0, 6: 0.05, 12: 0.10 },
  ...
  private getTrustSignals(gateway: Gateway, country?: string): string[] {
    ...
    if (data?.countries) signals.push(`Available in ${data.countries} countries`);
    
    // Local trust signals
    if (country && gateway === 'mercadopago' && ['BR', 'AR', 'MX'].includes(country)) {
  ...
  private getLocalizedName(gateway: Gateway, language?: string): string | undefined {
  ...
  private buildUserFriendlyExplanation(
    ...
    const config = DEFAULT_GATEWAY_CONFIGS[gateway];
    const primaryReason = factors[0]?.description || 'optimal for this transaction';
  ...
  private buildTechnicalExplanation(
    ...
    const factors = topGateway.factors.map(f => 
